...
const PopularOfWeek = ({ trend, genreMap }) => {
        ...
        >
          {canScrollRight && (
            <SliderBtn
              ...
              className="absolute right-5 top-1/2 -translate-y-1/2 sm:right-5"
            />
          )}
          ...
          {canScrollLeft && (
            <SliderBtn
              ...
              onClick={() => scroll("left")}
              className="absolute left-11.5 top-1/2 -translate-y-1/2 sm:left-20 max-sm:left-5"
            />
          )}
          ...
          {trend.map((item, index) => (
            <Link
              to={`/movie/${item.id}`}
              key={index}
              className="flex items-center gap-5 max-sm:min-w-[255px] sm:min-w-[325px] snap-center sm:snap-start cursor-pointer bg-transparent"
            >
              <div>
                <h1 className="dark:text-white text-black font-semibold sm:text-4xl max-sm:text-2xl">
                  {index + 1}
                </h1>
              </div>
              ...
              <div className="max-sm:min-w-[120px] max-sm:min-h-[100px] sm:min-w-[155px] sm:min-h-[155px] flex flex-col items-start justify-between max-sm:gap-0.5">
                <p className="bg-transparent dark:text-gray-300 dark:border dark:border-gray-400 text-gray-800 border-gray-800 text-xs text-center mb-5 max-sm:p-0.5 sm:p-1.5 rounded uppercase">
                  pg-13
                </p>
                <h4 className="dark:text-white text-black max-sm:text-sm">
                  {item.title}
                </h4>

                {item.genre_ids.slice(0, 2).map((id, idx) => (
                  <p
                    key={idx}
                    className="dark:text-gray-600 text-black sm:text-xs"
                  >
                    {genreMap[id]}
                    {idx < item.genre_ids.slice(0, 2).length - 1 ? ", " : ""}
                  </p>
                ))}
