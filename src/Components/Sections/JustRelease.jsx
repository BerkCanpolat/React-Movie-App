...
const JustRelease = ({ justRelease, genreMap, loadingMovie }) => {
  ...
  if (!justRelease || justRelease.length == 0) return null;
        ...
        >
          ...
          {canScrollLeft && (
            <SliderBtn
              ...
              onClick={() => scroll("left")}
              className="absolute left-11.5 top-1/2 -translate-y-1/2 sm:left-5"
            />
          )}

          {justRelease.map((movie, index) => (
            <Link
              to={`/movie/${movie.id}`}
              key={index}
                  ...
                  <div className="flex items-center gap-1.5">
                    ...
                    <p className="dark:text-white text-black text-sm">
                      ...
                      {movie.genre_ids.map((id, idx) => (
                        <span key={idx} className="dark:text-gray-500">
                          {` | ${genreMap[id]}`}
                        </span>
                      ))}
                    </p>
